...
int main(int argc, char* argv[])
{
    ...
    if ((sockfd = socket(AF_INET, SOCK_STREAM, 0)) < 0) //Ipv4, TCP -> sockfd 값이 양수가 나와야 함
        printf("socket error");
    ...
    servaddr.sin_port = htons(13); /* daytime server port number, htons -> host to network short */
    if (inet_pton(AF_INET, argv[1], &servaddr.sin_addr) <= 0)
        printf("inet_pton error for %s", argv[1]); // (argv[1] 문자열)에 저장된 dotted 
                                         // decimal 형식의 서버 주소를 32비트 이진수로 변환=network byte order로 바꾸다는 의미
